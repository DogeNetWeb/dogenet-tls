...
#      WITHOUT replacing the box's global resolvers. A dummy iface (dn-<tld>),
...
UNIT="dogenet-web-$TLD.service"
UNIT_PATH="/etc/systemd/system/$UNIT"
NFT_DIR="/etc/dogenet"
NFT_FILE="$NFT_DIR/nft-$TLD.nft"
IFACE="dn-$TLD"
SPLIT_FILE="$NFT_DIR/split-dns-$TLD.sh"
NM_UNMANAGED="/etc/NetworkManager/conf.d/dogenet-$TLD-unmanaged.conf"
CA_DEB="/usr/local/share/ca-certificates/dogenet-$TLD.crt"
CA_FED="/etc/pki/ca-trust/source/anchors/dogenet-$TLD.crt"

if [ -z "$CERT" ]; then
    CERT="$RHOME/.dogenet/dogenet-root-$TLD.crt"
fi
...
# ── systemd-resolved split-DNS on dummy dn-$TLD (routing-only ~$TLD) ──────────
...
write_split_dns() {
    ...
    cat > "$SPLIT_FILE" <<EOF
#!/bin/sh
# generated by dogenet-tls install-linux.sh — .$TLD split-DNS on $IFACE
set -eu
...
write_unit() {
    ...
    cat > "$UNIT_PATH" <<EOF
[Unit]
Description=DogeNet .$TLD split-DNS + loopback :443 redirect
...
ExecStop=-/usr/sbin/nft delete table ip dogenet-$TLD
...
apply_runtime() {
    ...
    if command -v nft >/dev/null 2>&1; then
        nft delete table ip dogenet-$TLD >/dev/null 2>&1 || true
...
# dn-<tld> and NXDOMAIN .pepe at Cloudflare — exclude the TLD from TRR. ────
FF_MARK="// dogenet-tls: trust OS roots (incl. the .$TLD root CA)"
...
firefox_install() {
    ...
    for pd in "$RHOME/.mozilla/firefox" \
        ...
        for prof in "$pd"/*/; do
            ...
            uj="${prof}user.js"
            grep -qs 'dogenet-tls: trust OS roots' "$uj" 2>/dev/null || printf '%s\n' "$FF_MARK" >> "$uj"
...
firefox_policies() {
    ...
    cp "$CERT" "/etc/firefox/policies/certificates/dogenet-$TLD.crt"
    chmod 644 "/etc/firefox/policies/certificates/dogenet-$TLD.crt"
    POL=/etc/firefox/policies/policies.json
    CERT_POL="/etc/firefox/policies/certificates/dogenet-$TLD.crt"
...
firefox_uninstall() {
    for pd in "$RHOME/.mozilla/firefox" \
        ...
        for prof in "$pd"/*/; do
            uj="${prof}user.js"
            [ -f "$uj" ] || continue
            sed -i '/dogenet-tls: trust OS roots/d' "$uj" 2>/dev/null || true
    ...
    rm -f "/etc/firefox/policies/certificates/dogenet-$TLD.crt"
    POL=/etc/firefox/policies/policies.json
    if [ -f "$POL" ] && command -v python3 >/dev/null 2>&1; then
        CERT_POL="/etc/firefox/policies/certificates/dogenet-$TLD.crt" TLD="$TLD" POL="$POL" python3 - <<'PY'
import json, os
path, cert, tld = os.environ["POL"], os.environ["CERT_POL"], os.environ["TLD"]
try:
    with open(path) as f:
...
manual verification:
  ...
  - nft list table ip dogenet-$TLD                     # :443 redirect (if nft worked)
  - awk -F: '/dogenet-$TLD/{print}' /etc/ca-certificates.conf  # Debian system store
EOF
}
...
do_uninstall() {
    ...
    command -v nft >/dev/null 2>&1 && nft delete table ip dogenet-$TLD 2>/dev/null || true
